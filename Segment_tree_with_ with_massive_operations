...
void relax(int v, int L, int M, int R) {
    tr[v] = tr[left_son(v)] + tr[right_son(v)] + gift[left_son(v)] * (M - L) + gift[right_son(v)] * (R - M);
}
 
void Push(int v = 0, int L = 0, int R = 0) {
    tr[v] += gift[v] * (R - L);
...
ll get(int l, int r, int v = 0, int L = 0, int R = n) {
    if (L == l && R == r)
        return tr[v] + gift[v] * (R - L);
    int M = (L + R) / 2;
    ll ans = 0;
    Push(v, L, R);
    if (M > l)
        ans += get(l, min(M, r), left_son(v), L, M);
    if (M < r)
        ans += get(max(l, M), r, right_son(v), M, R);
    return ans;
}
